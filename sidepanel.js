...
fillBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const { unmatchedStudents, pageOnlyStudents, absentStudents, totalStudents, marksEnteredCount, absentCount } = results[0].result;
    ...
    if (unmatchedStudents.length === totalStudents && totalStudents > 0) {
      ...
      displayUnmatchedStudents(unmatchedStudents, pageOnlyStudents, results[0].result.courseNo, results[0].result.section);
      return;
    }
...
function displayUnmatchedStudents(unmatchedStudents, pageOnlyStudents, courseNo = '', section = '') {
  ...
  let html = `
    <div class="unmatched-header">
      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        ...
        <h3 style="margin: 0; font-size: 14px;">Review Needed</h3>
        
        ${(courseNo || section) ? `
          <div class="course-section-chip">
            <span>${courseNo}${courseNo && section ? ' - ' : ''}${section}</span>
          </div>
        ` : ''}
      </div>

      <p class="unmatched-subtitle">Mismatch found between Excel and page.</p>
    </div>
  `;

  if (excelMisses.length > 0) {
    html += `
      <h4 style="margin:4px 0;color:#991b1b;font-size:12px;">In Excel, not on page (${excelMisses.length})</h4>
      ${table(excelMisses, ['ID', 'Name', 'Mark'])}
    `;
  }

  if (pageExtras.length > 0) {
    html += `
      <h4 style="margin:8px 0 4px;color:#991b1b;font-size:12px;">On page, not in Excel (${pageExtras.length})</h4>
      ${table(pageExtras.map(x => ({ id: x.id, name: x.name })), ['ID', 'Name'])}
    `;
  }
...
function displayAbsentStudents(absentStudents) {
  const container = document.getElementById("absentContainer") || createAbsentContainer();
  ...
  let html = `
    <div class="absent-header">
      <div style="display: flex; align-items: center; gap: 8px;">
        <div class="absent-icon">ℹ</div>
        <h3 style="margin: 0; font-size: 14px;">Absent Students (${absentStudents.length})</h3>
      </div>
      <p class="absent-subtitle">Toggle to assign "I" grade.</p>
    </div>
    <div class="absent-table-container">
      <table class="absent-table">
        ...
        <thead>
          <tr>
            <th>ID</th>
            <th>Name</th>
            <th style="text-align: center;">'I' Grade</th>
          </tr>
        </thead>
        <tbody>
  `;
  ...
  absentStudents.forEach((student, index) => {
    html += `
      <tr>
        ...
        <td><span class="student-name-text" title="${student.name}" style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:140px;">${student.name}</span></td>
        <td style="text-align: center;">
          <label class="switch">
            <input type="checkbox" class="fill-remarks-switch" data-student-id="${student.id}" data-index="${index}">
  ...
  const switches = container.querySelectorAll('.fill-remarks-switch');
    ...
    checkbox.addEventListener('change', async (e) => {
      ...
      try {
        ...
        await chrome.scripting.executeScript({
          target: { tabId: tabs[0].id },
          function: fillRemarksForAbsent,
          args: [studentId, isChecked]
        });
        ...
        // Error handling: Revert state
        checkbox.checked = !isChecked;
        checkbox.disabled = false;
        switchLabel.classList.add('error');
        setTimeout(() => switchLabel.classList.remove('error'), 2000);
      }
    });
  });
}
...
function createAbsentContainer() {
  const container = document.createElement('div');
  container.id = 'absentContainer';
  container.className = 'absent-container';
...
function fillRemarksForAbsent(studentId, shouldFill = true) {
  ...
  for (const studentInput of studentInputs) {
    const studentValue = studentInput.value || '';
    const match = studentValue.match(/^\s*([0-9]{4,})/);

    if (match && normId(match[1]) === normId(studentId)) {
      // Found the student row, now find the remarks field
      const rowRoot = studentInput.closest('.row') || studentInput.closest('formly-group') || studentInput.closest('.border-bottom') || studentInput.closest('[formly-field]');

      if (rowRoot) {
        // Look for remarks input field
        const remarksInput = rowRoot.querySelector('input[placeholder*="Remark" i], input[id*="remark" i], input[formcontrolname*="remark" i]');

        if (remarksInput) {
          remarksInput.value = shouldFill ? 'Absent in final (Grade I)' : '';
          remarksInput.dispatchEvent(new Event('input', { bubbles: true }));
          remarksInput.dispatchEvent(new Event('change', { bubbles: true }));

          // Visual feedback
          const remarksContainer = remarksInput.closest('.mdc-text-field, .mat-mdc-text-field-wrapper, .mat-mdc-form-field');
          if (remarksContainer) {
            if (shouldFill) {
              remarksContainer.style.backgroundColor = '#dbeafe';
              remarksContainer.style.borderColor = '#3b82f6';
            } else {
              remarksContainer.style.backgroundColor = '';
              remarksContainer.style.borderColor = '';
            }
            remarksContainer.style.borderRadius = '8px';
            remarksContainer.style.transition = 'all 0.3s ease';

            // Scroll into view smoothly
            remarksContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
...
async function fillMarksOnPage(data, selectedMarkIndex = 0) {
  try {
    ...
    // ---------- Meta Info extraction (Course, Section) ----------
    let courseNo = '';
    let section = '';
    ...
    for (let r = 0; r < Math.min(data.length, 15); r++) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (cell.includes('course no')) {
          courseNo = String(row[c + 1] || '').trim();
        }
        if (cell.includes('section')) {
          section = String(row[c + 1] || '').trim();
        }
      }
    }
    ...
    // Detect 'Total' columns (e.g., "Total", "Total Mark", "Total (Rounded)")
    const totalColumns = [];
    for (let i = 0; i < headerRow.length; i++) {
      const h = String(headerRow[i] || '').trim().toLowerCase();
      // Match "total" but not "subtotal"
      if (h.includes('total') && !h.includes('subtotal')) {
        totalColumns.push(i);
      }
      if (h === 'final') finalColumnIndex = i;
    }
    ...
    // ---------- Extract "Full Mark" from meta row or header ----------
    let fullMark = 100;

    // Fallback 1: Try to find full mark in the header text itself, e.g., "Total (100)"
    const headerText = String(headerRow[totalColumnIndex] || '');
    const headerMatch = headerText.match(/\(([\d.]+)\)/);
    if (headerMatch) {
      fullMark = parseFloat(headerMatch[1]);
    } else {
      // Fallback 2: Look at the meta row (usually row below header)
      const metaRow = data[headerRowIndex + 1] || [];
      const metaCell = metaRow[totalColumnIndex];
      if (metaCell != null) {
        let rawVal = String(metaCell);
        let num = parseFloat(rawVal.replace(/[^\d.]/g, ''));
        if (!isNaN(num) && num > 0) {
          // If the metaCell looks like a percentage (e.g. 1.0 for 100%), scale it up.
          // Common case: Excel returns 1 for 100% or 0.5 for 50%.
          if (num <= 1 && (rawVal.includes('%') || (typeof metaCell === 'number' && metaCell <= 1))) {
            num *= 100;
          }
          fullMark = num;
        }
  ...
  `;
    ...
    const absentStudents = [];      // students marked as absent
    ...
    for (const rec of excelById.values()) {
      ...
      if (pageRow) {
        if (rec.isAbsent) {
          ...
          // Track absent student with their ID
          absentStudents.push({ id: rec.id, name: rec.name || 'Unknown' });
    ...
    return {
      ...
      absentStudents,              // Students marked as absent
